                  ...
                  <v-table fixed-header class="text-no-wrap" hide-default-footer>
                    ...
                    <tbody>
                      <tr v-for="(transaction, index) in items" :key="index">
                        ...
                        <td>{{ transaction.raw.orderDate ? formatDateDisplay(transaction.raw.orderDate) : '-' }}</td>
  ...
  try {
    ...
    if (filterStartDate.value) {
      ...
      filtered = filtered.filter(t => {
        const orderDate = dayjs(t.orderDate)
        return orderDate.isValid() && orderDate.isSameOrAfter(startDate, 'day')
      })
    }
    ...
    if (filterEndDate.value) {
      ...
      filtered = filtered.filter(t => {
        const orderDate = dayjs(t.orderDate)
        return orderDate.isValid() && orderDate.isSameOrBefore(endDate, 'day')
      })
    }
