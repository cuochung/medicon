<template>
  <div class="supplier-list pa-6">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="supplier-hero" elevation="0" rounded="xl">
            <div class="d-flex flex-column flex-md-row align-center justify-space-between">
              <div class="d-flex align-center mb-4 mb-md-0">
                ...
                <div>
                  <h2 class="hero-title mb-1">廠商管理面板</h2>
                  <p class="hero-subtitle mb-0">維護廠商資料，掌握廠商資訊。</p>
                </div>
              </div>
      ...
      <v-row class="summary-row" dense>
        <v-col cols="12" sm="6" md="4">
          <v-card class="summary-card" variant="tonal" color="primary">
            <div class="summary-card__title">廠商總數</div>
            <div class="summary-card__value">{{ totalCount }}</div>
            <div class="summary-card__caption">目前已建立的廠商數量</div>
          </v-card>
        </v-col>
        <v-col cols="12" sm="6" md="4">
          <v-card class="summary-card" variant="tonal" color="secondary">
            ...
            <div class="summary-card__caption">已填寫聯絡電話的廠商</div>
          </v-card>
        </v-col>
        <v-col cols="12" md="4">
          <v-card class="summary-card" variant="tonal" color="info">
            <div class="summary-card__title">外商</div>
            <div class="summary-card__value">{{ foreignSupplierCount }}</div>
            <div class="summary-card__caption">標記為外商的廠商</div>
          </v-card>
        </v-col>
      </v-row>
      ...
      <v-row class="mt-4">
        <v-col cols="12">
          <v-sheet class="supplier-toolbar" elevation="0" rounded="xl">
            <v-row align="center" no-gutters>
              <v-col cols="12" md="8">
                <v-text-field v-model="searchKey" label="搜尋廠商關鍵字" density="comfortable" variant="outlined"
              ...
              <v-col cols="12" md="4" class="d-flex justify-end mt-3 mt-md-0">
                <importSupplier class="mr-md-2" @getAllData="getAllData"></importSupplier>
                <!-- <popupadd v-if="auth.supplier_add_key" ref="childFn" :authKeys="authKeys" :supplierItems="items"
      ...
      <v-card class="supplier-table-card">
        <v-card-text>
                ...
                <template #default>
                  <thead class="title text-h6">
                    <tr>
                      ...
                      <th class="text-left">廠商編號</th>
                      <th class="text-left">廠商全稱</th>
                      <th class="text-left">統一編號</th>
                      <th class="text-left">負責人</th>
                      <th class="text-left">聯絡人</th>
                      <th class="text-left">聯絡電話一</th>
                      <th class="text-left">聯絡電話二</th>
                      <th class="text-left">行動電話</th>
                      <th class="text-left">電子郵件</th>
                      <th class="text-left">地址</th>
                      <th class="text-left">採購員</th>
                      <th class="text-left">外商</th>
                    ...
                    <tr v-for="(item, index) in items" :key="index">
                      <td>
                        <!-- <v-menu transition="scale-transition" offset-y>
                          ...
                          <v-list>
                            <v-list-item @click="edit(item.raw)" v-if="auth.supplier_edit_key">
                              <v-list-item-title>修改</v-list-item-title>
                            </v-list-item>
                            <v-list-item @click="del(item.raw)" v-if="auth.supplier_del_key">
                              <v-list-item-title>刪除</v-list-item-title>
                            </v-list-item>
                          </v-list>
                        </v-menu> -->
                      </td>
                      <td>{{ item.raw.supplierNumber || '-' }}</td>
                      <td>{{ item.raw.supplierFullName || '-' }}</td>
                      <td>{{ item.raw.unifiedNumber || '-' }}</td>
                      <td>{{ item.raw.personInCharge || '-' }}</td>
                      <td>{{ item.raw.contactPerson || '-' }}</td>
                      <td>{{ item.raw.contactPhone1 || '-' }}</td>
                      <td>{{ item.raw.contactPhone2 || '-' }}</td>
                      <td>{{ item.raw.mobilePhone || '-' }}</td>
                      <td>{{ item.raw.email || '-' }}</td>
                      <td>
                        <div class="text-truncate" style="max-width: 200px" :title="item.raw.address">
                          {{ item.raw.address || '-' }}
                        </div>
                      </td>
                      <td>{{ item.raw.purchaser || '-' }}</td>
                      <td>{{ item.raw.isForeignSupplier ? '是' : '否' }}</td>
                      <td>
...
import importSupplier from "./importSupplier.vue"
...
// 響應式數據
const items = ref([])
const usingDatabase = ref("supplier")
const searchKey = ref("")
const auth = ref("")
...
const withContactPhoneCount = computed(() => 
  items.value.filter((item) => item?.contactPhone1 || item?.contactPhone2 || item?.mobilePhone).length
)
const foreignSupplierCount = computed(() => 
  items.value.filter((item) => item?.isForeignSupplier).length
)
  ...
  await api.get(usingDatabase.value).then((rs) => {
    if (rs.length > 0) {
      ...
      items.value.sort((a, b) => {
        const numA = a.supplierNumber || ''
        const numB = b.supplierNumber || ''
...
// 表格操作按鈕 hover
.supplier-table-card :deep(.v-btn) {
